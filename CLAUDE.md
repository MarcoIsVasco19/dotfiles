...
This is a personal dotfiles repository containing shell and editor configurations. The repository includes:

- **Shell configuration** (.bashrc, .bash_profile) - Bash environment setup with custom aliases and tool integrations
- **Neovim configuration** (.config/nvim) - Lua-based Neovim setup using lazy.nvim

## Repository Structure

```
.
├── .bashrc              # Main bash configuration (sourced by .bash_profile)
├── .bash_profile        # macOS-specific bash profile setup
└── .config/
...
Primary bash configuration file with environment setup and aliases. Key components:

**Dependencies:**
- Expects `~/.git-prompt.sh` for git prompt integration
...
- `tf='terraform'`, `tg='terragrunt'`, `tm='terramate'` - IaC tools
- `k='kubectl'`, `kctx='kubectx'` - Kubernetes tools
- `repos='cd ~/repos'` - Quick navigation
- `ctg` - Clean terragrunt cache recursively

**Kubernetes Configuration:**
- Merges multiple kubeconfigs from ~/.kube/ into combined-config
- Uses kubectl bash completion for `k` alias
- Docker command aliased to podman

**Tool Integrations:**
- GitHub Copilot CLI via `gh copilot alias`
- Terraform, Terragrunt, Terramate bash completion
- ASDF completion

### .bash_profile

macOS-specific initialization that:
- Sets up Homebrew environment via `brew shellenv`
- Sources .bashrc for shared configuration
- Enables Homebrew bash completion
- Configures ASDF paths
...
1. `.config/nvim/init.lua` - Entry point that loads core settings and lazy.nvim
...
**Global Keymaps (Leader key: Space):**
...
- **General**:
  ...
  - `<leader>Q` - Quit all windows (close Neovim)
...
1. **Shell changes** - Edit .bashrc for cross-platform settings, .bash_profile for macOS-specific setup
2. **Neovim plugin additions** - Add new plugin files in `lua/marco/plugins/`
3. **Neovim core settings** - Modify files in `lua/marco/core/`
4. **LSP configurations** - Edit files in `lua/marco/plugins/lsp/`

The configuration uses a namespace pattern (`marco.*`) for all Neovim Lua modules to avoid conflicts with other plugins.
